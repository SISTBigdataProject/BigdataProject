...
/*
 GradeMovie[] glist=new GradeMovie[map.size()];
	for (int k = 0; k < map.size(); k++) {
		glist[k] = new GradeMovie(clist[k],tlist[k]);
		glist[k].start();
		if (k % 300 == 0 && k != 0) {
			for (int j = 0; j < k; j++) {
				while (glist[j].isAlive());
			}
		}
		
			System.out.println(k+" 쓰레드 종료");
			for(GradeMovieVO gvo:glist[k].grlist)
			{
			mm.dao.GradeMovieInsert(gvo);
			}
		}catch (Exception e) {
	...
	System.out.println("==========================GradeMovie end==========================");
*/
...
/*
	  ArrayList<MovieVO> links=mm.naverYearLinkData();
	  int k=links.size();
	  for(int i=0;i<k;i++){
		  MovieDetail[] dlist=new MovieDetail[k];
		  dlist[i]=new MovieDetail(links.get(i).getLink());
		  dlist[i].start();
	  if (i % 10000 == 0 && i != 0) {
			for (int j = 0; j < i; j++) {
				while (dlist[j].isAlive());
			}
		} 
		try
		{
			dlist[i].join();
			System.out.println(i+" 쓰레드 종료");
			for(MovieVO dvo: dlist[i].dlist){
				mm.dao.MovieInsert(dvo);
			}
		}catch (Exception e) {
			System.out.println(e.getMessage());
		}
 ...
 public ArrayList<MovieVO> naverYearLinkData(){
		...
		try{
			...
			for(int i=0;i<35;i++)
			{
				...
				page[i]=(int)(Integer.parseInt(pcount[i].substring(pcount[i].lastIndexOf("(")+1, pcount[i].lastIndexOf(")")))/20.0)+1;
			...
			for(int i=1;i<=29;i++)
			{
				...
				for(int j=1;j<=page[i];j++){
					Document doc=Jsoup.connect("https://movie.naver.com/movie/sdb/browsing/bmovie.nhn?open="+pyear[i]+"&page="+j).get();
