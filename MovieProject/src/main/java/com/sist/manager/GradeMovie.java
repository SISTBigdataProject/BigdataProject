...
public class GradeMovie extends Thread {
	...
	public void run() {
		...
		try {
			...
			String grlink = "";
			// 최신순
			grlink = "https://movie.naver.com/movie/bi/mi/pointWriteFormList.nhn?code=" + code
					+ "&type=after&onlyActualPointYn=N&order=newest&page=1";
			// 호감순
			...
			try {
				...
				try {
					for (int k1 = 0; k1 < p; k1++) // p
					{
						...
						String grmon = "";
						grlink = "https://movie.naver.com/movie/bi/mi/pointWriteFormList.nhn?code=" + code
								+ "&type=after&onlyActualPointYn=N&order=newest&page=" + (k1 + 1);
